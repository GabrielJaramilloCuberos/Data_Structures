...
/**********************************************************************************************************************
 * Implementacion de grafos por Salomon Avila y Gabriel Jaramillo                                                     *
...
public:
  ...
  //Destructor de memoria por defecto
  ~Euler()      { delete [] adj;  }
  //Adiciona una arista al grafo
  void addEdge(int u, int v) {  adj[u].push_back(v); adj[v].push_back(u); }
  //Remueve una arista del grafo
  void rmvEdge(int u, int v);
  //Impresion del camino de euler
  void printEulerTour();
  //Funcion auxiliar para la impresion del camino de euler
  void printEulerUtil(int s);
  //Contador de los nos nodos visitados en el DFS (Depth First Search)
  int DFSCount(int v, bool visited[]);
  //Revisa si la siguiente arista es valida para el camino
  bool isValidNextEdge(int u, int v);
  //Imprime si es o no un camino de euler
  int iseuler(int[]);
};
...
int main()
{
	...
	for(i=1;i<=Edges;i++)
 	...
 	cout<<"\n";
 	if(g1.iseuler(a) == 1){
    //cout<<"Hasta aca está perfecto"<<endl;
 		g1.printEulerTour();
    //cout<<"Aca sigue bien"<<endl;
  }
  return 0;
}